      ...
      <ul className={css.mindMapList}>
        {cards.map((card, index) => (
          <li key={index} style={{ display: index === currentSlideApe ? 'block' : 'none' }}>
            <div className={css.mindMapItem}> <img className={css.collectionCard} src={card} alt="ewioewij" /> </div>
          </li>
        ))}
      </ul>
      <span className={css.mindMapBtns}>
        <button className={css.mindMapPrevBtn} disabled={currentSlideApe === 0} onClick={handlePrevClick}>
          <p>Prev</p>
        </button>
        <button className={css.mindMapNextBtn} disabled={currentSlideApe === cards.length - 1} onClick={handleNextClick}>
          <p>Next</p>
        </button>
      </span>
    </div>
